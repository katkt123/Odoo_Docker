...
class ProductTemplate(models.Model):
    ...
    def _search_get_detail(self, website, order, options):
        ...
        # Custom filters
        filter_type_id = _safe_int(_first(args.get('filter_type_id', '')))
        if filter_type_id:
            extra_domain.append(('type_id', '=', filter_type_id))
        ...
        if isinstance(base_domain, list):
            base_domain.append(extra_domain)
        elif base_domain:
            detail['base_domain'] = [base_domain, extra_domain]
        else:
