...
class SmileLivingShop(WebsiteSale):
    ...
    def _get_shop_domain(self, search_term, category, attribute_value_dict):
        ...
        # Always restrict /shop to real-estate products by requiring a linked smileliving.property.
        # We also keep the website company scoping consistent with earlier behavior.
        ...
        # Only enforce the restriction when at least one property exists, or when the user actively filters.
        ...
        return domain + [('id', 'in', product_ids)]
    ...
    def _get_search_domain(self, search, category, attrib_values):
        ...
        SmileLiving filtering is handled via `_get_shop_domain` using `smileliving.property`.
        """
        ...
        prop_domain = [('company_id', 'in', [website_company.id, False])]
        ...
        if filter_type_id:
            try:
                ...
                prop_domain.append(('type_id', '=', int(filter_type_id)))
            except (ValueError, TypeError):
                pass

        # Filter theo trạng thái
        if filter_status:
            prop_domain.append(('house_status', '=', filter_status))

        # Filter theo diện tích
        if filter_area_min:
            try:
                prop_domain.append(('area', '>=', float(filter_area_min)))
            except (ValueError, TypeError):
                pass
        if filter_area_max:
            try:
                prop_domain.append(('area', '<=', float(filter_area_max)))
            except (ValueError, TypeError):
                pass
        
        # (Removed) price filtering
        
        prop_recs = request.env['smileliving.property'].sudo().search(prop_domain)
        domain.append(('id', 'in', prop_recs.mapped('product_tmpl_id').ids))
        return domain
    ...
    def _get_products_domain(self, search, category, attrib_values, **kwargs):
        ...
        if tinhthanh_id:
            try:
                prop_domain.append(('tinhthanh_id', '=', int(tinhthanh_id)))
                print(f"DEBUG: Filter theo tinhthanh_id = {tinhthanh_id}")
            except (ValueError, TypeError):
                pass
        if quanhuyen_id:
            try:
                prop_domain.append(('quanhuyen_id', '=', int(quanhuyen_id)))
                print(f"DEBUG: Filter theo quanhuyen_id = {quanhuyen_id}")
            except (ValueError, TypeError):
                pass
        if phuongxa_id:
            try:
                prop_domain.append(('phuongxa_id', '=', int(phuongxa_id)))
                print(f"DEBUG: Filter theo phuongxa_id = {phuongxa_id}")
            except (ValueError, TypeError):
                pass
