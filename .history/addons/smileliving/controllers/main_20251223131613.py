...
class SmileLivingController(http.Controller):
    ...
    def _map_property_to_company(self, property_tmpl, company):
        """Map an incoming product.template to the equivalent record in the given company.

        This avoids multi-company record rule errors when URLs or IDs point to templates
        owned by another company (e.g. legacy links).
        """
        if not property_tmpl or not property_tmpl.exists():
            return property_tmpl

        if property_tmpl.company_id and property_tmpl.company_id.id == company.id:
            return property_tmpl

        PT = request.env['product.template'].sudo()

        # Prefer mapping by Internal Reference (default_code) if available.
        code = property_tmpl.product_variant_id.default_code
        if code:
            target_code = code
            if company.currency_id and company.currency_id.name == 'VND' and not code.endswith('-VN'):
                target_code = f"{code}-VN"
            elif company.currency_id and company.currency_id.name != 'VND' and code.endswith('-VN'):
                target_code = code[:-3]
        ...
        # Fallback: match by base name (strip trailing parenthetical suffix like '(bản sao)' / '(copy)')
        base_name = re.sub(r'\s*\([^)]*\)\s*$', '', property_tmpl.name or '').strip()
        if base_name:
    ...
    def homepage(self, **kwargs):
        ...
        try:
            website_company = self._get_website_company()
            # Lấy bất động sản từ product.template
            properties = request.env['product.template'].sudo().with_company(website_company).search([
                ('is_house', '=', True),
    ...
    def property_detail(self, property_id, **kwargs):
        ...
        property = self._map_property_to_company(property, website_company)
        if not property:
            return request.not_found()
