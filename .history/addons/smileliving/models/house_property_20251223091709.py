...
class HouseProduct(models.Model):
    ...
    def _sync_categories_from_type(self):
        """Keep product categories aligned with the selected real-estate type.

        - Sales Category: product.template.categ_id
        - Website Category: product.template.public_categ_ids
        """
        for rec in self:
            ...
            vals = {}
            if rec.type_id.product_categ_id and rec.categ_id != rec.type_id.product_categ_id:
                vals['categ_id'] = rec.type_id.product_categ_id.id

            if rec.type_id.public_categ_id:
                current_public = rec.public_categ_ids
                target_public = rec.type_id.public_categ_id
                if not current_public or (len(current_public) != 1) or (current_public.id != target_public.id):
                    vals['public_categ_ids'] = [(6, 0, [target_public.id])]

            if vals:
                # Avoid recursion by writing only when needed
                super(HouseProduct, rec).write(vals)

    @api.onchange('type_id')
    def _onchange_type_id_sync_categories(self):
        for rec in self:
            rec._sync_categories_from_type()
