    ...
    def _search_get_detail(self, website, order, options):
        ...
        def _to_domain(dom):
            ...
            if isinstance(dom, Domain):
                return dom
            # In some cases base_domain comes as a list of sub-domains (nested lists)
            # rather than a single flat domain list.
            if isinstance(dom, (list, tuple)):
                has_leaf_or_token = any(
                    isinstance(item, str)
                    or (
                ...
                if not has_leaf_or_token and all(isinstance(item, (Domain, list, tuple)) for item in dom):
                    parts = []
                    for item in dom:
                        if isinstance(item, Domain):
                            parts.append(item)
                        else:
                            parts.append(Domain(item))
                    return Domain.AND(parts) if parts else Domain([])
            return Domain(dom)

        base_domain = detail.get('base_domain')
        try:
            base = _to_domain(base_domain)
            detail['base_domain'] = Domain.AND([base, Domain(extra_domain)])
        except Exception:
            # Never crash the shop search; keep original base_domain if it can't be normalized.
            # Fallback still applies SmileLiving domain when possible.
            try:
                detail['base_domain'] = Domain.AND([Domain(extra_domain), _to_domain(base_domain)])
            except Exception:
